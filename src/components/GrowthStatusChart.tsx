...
export function GrowthStatusChart({ baby, measurements, noWrapper = false, className = "" }: GrowthStatusChartProps) {
  ...
  const data = useMemo(() => {
    const values = sortedMeasurements.map((measurement) => {
      if (metric === "weight") {
    ...
    const label = metric === "weight" ? "Weight (kg)" : "Height (cm)";
    const color = metric === "weight" ? "#2563eb" : "#0f766e";
  ...
  }, [labels, metric, sortedMeasurements]);
    ...
    <div className={wrapperClass}>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-lg font-semibold text-slate-900">{metric === "weight" ? "Weight" : "Height"} over time</p>
        </div>
        <div className="inline-flex rounded-full border border-slate-200 bg-white p-1 shadow-sm">
          <button
            type="button"
            onClick={() => setMetric("weight")}
            className={`rounded-full px-4 py-2 text-sm font-semibold transition ${metric === "weight" ? "bg-slate-900 text-white" : "text-slate-600 hover:text-slate-900"}`}
          >
            Weight
          </button>
          <button
            ...
            className={`rounded-full px-4 py-2 text-sm font-semibold transition ${metric === "height" ? "bg-slate-900 text-white" : "text-slate-600 hover:text-slate-900"}`}
          >
            Height
          </button>
        </div>
