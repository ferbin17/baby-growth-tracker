...
export function HeightChart() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const whoSeries = await loadWhoData(profile.gender, "height");
      const series = buildGrowthSeries(profile, rows, whoSeries, "height");
  ...
  if (!baby || data.length === 0) {
    return null;
  }

  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      <h3 className="text-lg font-semibold text-slate-900">Height chart</h3>
      <div className="mt-4 h-72 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data}>
            ...
            <Line type="monotone" dataKey="actual" stroke="#8b5cf6" dot={{ r: 3 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
