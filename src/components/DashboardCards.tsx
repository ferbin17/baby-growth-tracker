...
export function DashboardCards() {
  ...
  const latestWeightPercentile = useMemo(() => {
    ...
    return calculatePercentileFromMeasurement(
      latest.weightKg,
      point
    );
  }, [latest, whoWeight]);
  ...
  const latestWeight =
    latest.weightKg ?? baby.birthWeightKg;

  const latestHeight =
    latest.heightCm ?? baby.birthHeightCm;

  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <MetricCard
        title="Current age"
        value={getAgeLabel(birthDate, new Date())}
      />

      <MetricCard
        title="Latest weight"
        value={formatWeightLabel(latestWeight)}
      />

      <MetricCard
        title="Latest height"
        value={formatHeightLabel(latestHeight)}
      />
