...
export function interpolateWhoPoint(series: WhoPoint[], ageDays: number): WhoPoint {
  if (!series.length) {
...
export function calculatePercentileFromMeasurement(value: number, point: WhoPoint): number {
...
export function calculateZScore(value: number, point: WhoPoint): number {
  const { l, m, s } = point;
...
export function expectedValueAtPercentile(point: WhoPoint, percentile: number): number {
...
export async function loadWhoData(gender: Gender, metric: Metric): Promise<WhoPoint[]> {
  const fileName = `${gender}-${metric}.json`;

  const response = await fetch(`/who/${fileName}`);

  if (!response.ok) {
    throw new Error(`Unable to load WHO data: ${fileName}`);
  }

  const data = (await response.json()) as WhoPoint[];

  if (!Array.isArray(data) || data.length === 0) {
    throw new Error(`Invalid WHO data: ${fileName}`);
  }
...
function normalCDF(x: number): number {
  ...
  const prob =
    1 - d * t * (0.3193815 + t * (-0.3565638 + t * (1.7814779 + t * (-1.821255 + t * 1.330274))));
...
function inverseNormalCDF(p: number): number {
  ...
  const a = [
    -39.696830286, 220.946098424, -275.928510446, 138.357751867, -30.664798066, 2.506628245,
  ];

  const b = [-54.476098798, 161.585836858, -155.698979859, 66.801311887, -13.280681553];

  const c = [
    -0.007784894002, -0.322396458041, -2.400758277161, -2.549732539343, 4.374664141977,
    2.938163982138,
  ];

  const d = [0.007784695709, 0.32246712907, 2.445134137143, 3.754408661907];
